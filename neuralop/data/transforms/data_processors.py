...
class SubsamplingGINODataProcessor(DataProcessor):
    ...
    """
    def __init__(
        self, in_normalizer=None, out_normalizer=None, positional_encoding=None, subsampling_level=None
    ):
        """A simple processor to pre/post process data before training/inferencing a model.
        ...
        positional_encoding : Processor, optional, default is None
            class that appends a positional encoding to the input
        subsampling_level : float, optional, default is None
            level at which to subsample points
        """
        super().__init__()
        self.in_normalizer = in_normalizer
        self.out_normalizer = out_normalizer
    ...
    def preprocess (self, data_dict, batched=True):
        ...
        n_in = int(input_geom.shape[1] * self.subsampling_level)
        input_indices = random.sample(list(range(input_geom.shape[0])), k=n_in)
        x = x[:, input_indices, ...]
        ...
        y = data_dict["y"].to(self.device)
        n_out = int(output_queries.shape[1] * self.subsampling_level)
        output_indices = random.sample(list(range(output_queries.shape[0])), k=n_out)
        y = y[:, output_indices, ...]
